...
def convert_cflow_to_callee_dot(cflow_file, dot_file, rankdir="TB", node_shape="box", node_color="lightcoral", 
                                font_name="Helvetica", font_size=10, arrow_color="black", arrow_size=0.7):
    """Convert a cflow output file to a .dot file for a callee graph with custom styling."""
    with open(cflow_file, 'r') as f, open(dot_file, 'w') as out:
        ...
        for line in f:
            ...
            # Reverse the direction for callee graph: function points to the function it calls
            if stack:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python3 cflow_to_callee_dot.py <input_cflow_file> <output_dot_file> [rankdir] [node_shape] [node_color] [font_name] [font_size] [arrow_color] [arrow_size]")
        sys.exit(1)
    ...
    convert_cflow_to_callee_dot(input_cflow_file, output_dot_file, rankdir, node_shape, node_color, font_name, font_size, arrow_color, arrow_size)
